...
import { lengthDirectiveTransformer } from './graphQlDirectives/lengthDirective';
import { modelDirectiveTransformer } from './graphQlDirectives/modelDirective';
import {
  mapSchema,
  getDirective,
  MapperKind 
    ...
    [MapperKind.OBJECT_TYPE]: (objectConfig: GraphQLObjectType) => {
      ...
      if (modelDirective) {
        ...
        for (const [fieldName, fieldDefinition] of Object.entries(objectConfig.getFields())) {
          switch(fieldDefinition.type.toString()) {
            case 'ceramicStringType!': // TODO: Is there a better way of introspecting the type?
            case 'ceramicStringType':
